...
const filteredSuggestions = computed(() => {
  if (!normalizedValue.value) {
    return cleanSuggestions.value.slice(0, 8);
  }

  return cleanSuggestions.value
...
const hasExactMatch = computed(() => {
  ...
  return cleanSuggestions.value.some((item) => {
    return item.toLowerCase() === normalizedValue.value;
  });
});
