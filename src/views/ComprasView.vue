...
<template>
  <div class="h-full flex flex-col pt-4 px-4 sm:px-8 pb-32 md:pb-8">
    <div class="flex justify-between items-center mb-6">
      ...
      <button @click="isNewFormOpen = true" class="hidden md:flex items-center gap-2 px-4 py-2 bg-accent text-main-dark font-medium rounded-xl hover:bg-opacity-90 transition-all shadow-sm">
    ...
    <!-- Filters & Actions -->
    <div class="flex flex-col md:flex-row gap-4 mb-6 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
      <!-- Search -->
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        ...
        >
          ...
          <div class="mt-auto pt-3 border-t border-gray-50 flex flex-col gap-2 z-10">
             <div class="text-[10px] text-gray-400">Req: {{ req.email }}</div>
            <div class="flex items-center justify-between text-xs text-gray-500">
              <div class="flex items-center gap-1.5">
    ...
    <Teleport to="body">
      <NuevoFormularioCompra 
        ...
        @created="store.fetchSolicitudes()"
      />
    </Teleport>
  </div>
</template>
